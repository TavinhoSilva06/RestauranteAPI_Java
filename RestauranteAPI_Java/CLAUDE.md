...
RestauranteAPI_Java é uma API REST em Spring Boot 4.1.0 (Java 21), com MongoDB. **Módulo implementado**: cadastro e autenticação de clientes com JWT stateless (implementado manualmente, sem biblioteca JWT). **Módulos planejados mas não implementados**: cardápio e pedidos (ver `src/main/java/com/example/Restaurante/docs/roadmap.md`).
...
- **`security/`** — `JwtService` (JWT HS256 manual), `JwtAuthenticationFilter`, `ClienteUserDetails(Service)`.
...
3. **Requisições protegidas**: cliente envia header `Authentication: <token>`. `JwtAuthenticationFilter` valida e popula `SecurityContextHolder`.

## Pontos de atenção

- JWT manual (sem biblioteca) — considerar migração a jjwt/Nimbus se projeto crescer.
- Sem autorização por papel (`Papel` tem 3 valores, nenhuma rota checa role).
- `jwt.secret` hardcoded em `application.properties` (valor de dev, commitado).
- `Passos/` e `explicacoes/` documentam design antigo (cookie/sessão) — desatualizadas.
- Header não-padrão: `Authentication` em vez de `Authorization: Bearer`.
